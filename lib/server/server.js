...
(function() {
  var Event, Session, allowCrossDomain, app, eventSchema, express, log, logAccess, logger, mongoose, sessionSchema;
  ...
  app.post('/api/events/:eventId/session', function(request, response) {
    ...
    return Event.findById(request.params.eventId).exec(function(error, event) {
      if (error) {
        return response.json({
