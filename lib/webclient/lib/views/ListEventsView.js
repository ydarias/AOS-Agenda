...
(function() {
    ...
    function getEventHtml(event) {
        return WeEvent.template("event", {
            ...
            startDate: moment(event.startDate).format(WeEvent.Configuration.parseDatePickerFormat),
            endDate: moment(event.endDate).format(WeEvent.Configuration.parseDatePickerFormat),
            id: event._id
        });
    }
